...
# update_project-51-pdf-generator-merge-with-update49-redesign.sh
#
# Чинит регрессию: предыдущий update_app.sh (update 47) перезаписал
# InvoicePdfGenerator.kt СВОЕЙ версией файла, откатив весь редизайн
# из update_project-49-invoice-pdf-redesign.sh (лого в полном разрешении,
# починенная таблица VAT, формат номера Numer/MM/RRRR, палитра из colors.xml).
#
# Этот скрипт кладёт ЕДИНСТВЕННЫЙ файл — InvoicePdfGenerator.kt — в котором
# редизайн update 49 СОХРАНЁН полностью, а поверх него применены фиксы
# из update 47:
#   - заголовок документа всегда 'FAKTURA' (не 'RACHUNEK')
#   - плашка продавца 'Osoba fizyczna prowadząca działalność nierejestrowaną
#     (bez NIP)'
#   - исправленный стиль/позиция плашки покупателя
#   - блок 'Podstawa prawna zwolnienia z VAT'
#   - функция generateCorrection() для модуля Korekta (используется
#     AddInvoiceCorrectionActivity.kt из update 47 — она уже в проекте
#     и её трогать не нужно)
#
# Никакие другие файлы (AppDatabase.kt, VatComplianceHelper.kt, SettingsTaxActivity.kt
# и т.д.) этот скрипт не трогает — они не пострадали от update 47 и уже в порядке.
...
BACKUP_DIR=".update51_backup_$(date +%Y%m%d_%H%M%S)"
...
cat > "app/src/main/java/com/example/fa_ksiegowy/InvoicePdfGenerator.kt" << 'FA_KSIEGOWY_UPDATE51_EOF'
...
object InvoicePdfGenerator {
     ...
     * plus przyczynę korekty podaną przez użytkownika.
     */
...
FA_KSIEGOWY_UPDATE51_EOF

git add .
echo "Готово. InvoicePdfGenerator.kt обновлён (update49-редизайн + фиксы update47 слиты воедино)."
echo "git commit -m 'update 51: merge PDF redesign (49) with FAKTURA/legal-notes fixes (47)' && git push"
