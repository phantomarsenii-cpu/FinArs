...
# ============================================================================
# FinArs — update_project-XX-revenuecat-multistore.sh
#
# Интеграция RevenueCat SDK (единый Pro-доступ Monthly/Yearly, Test Store)
# + поддержка Google Play И Samsung Galaxy Store с автоопределением магазина
# установки во время выполнения (StoreDetector.kt).
#
# Что делает скрипт:
#   1. Бэкапит все файлы, которые будут перезаписаны (с таймстампом).
#   2. Перезаписывает/создаёт Kotlin-файлы: StoreSource, StoreDetector,
#      SubscriptionService, BillingManager (теперь фасад над RevenueCat),
#      FaApp (инициализация SDK), SettingsProActivity (кнопка "Восстановить
#      покупки" + мгновенный refresh UI), PurchaseVerifier (помечен как
#      неиспользуемый — валидацию покупок теперь делает RevenueCat).
#   3. Обновляет layout экрана Pro (кнопка восстановления покупок) и
#      строки локализации (values / values-ru / values-pl).
#   4. Обновляет app/build.gradle: добавляет зависимости RevenueCat
#      (purchases + purchases-store-galaxy), убирает прямую зависимость
#      от com.android.billingclient (больше не используется напрямую).
#   5. Делает лёгкую проверку синтаксиса (баланс скобок) изменённых файлов.
#   6. Выполняет git add / commit / push origin main — сборка APK
#      запустится в GitHub Actions.
#
# Идемпотентность: скрипт всегда перезаписывает файлы целиком актуальным
# содержимым — повторный запуск безопасен и не создаёт дублей.
#
# ВАЖНО ПОСЛЕ ЭТОГО ОБНОВЛЕНИЯ:
#   - Сейчас используется ТОЛЬКО тестовый ключ RevenueCat (Test Store):
#     Test_BFXzgXddRkopsjEDdnaRTNtVXuY — работает одинаково на любом
#     устройстве/эмуляторе, независимо от реального магазина установки.
#   - Когда протестируете покупки через Test Store и получите боевые
#     публичные ключи (Project settings -> API keys) для Google Play
#     (goog_...) и Galaxy Store (galx_...) — впишите их в
#     SubscriptionService.kt в константы GOOGLE_PLAY_API_KEY и
#     GALAXY_STORE_API_KEY. До тех пор, пока они пустые, приложение
#     ВСЕГДА использует Test Store, даже если реально установлено из
#     Google Play или Galaxy Store — это сделано намеренно, чтобы
#     исключить случайные боевые вызовы без настроенного ключа.
#   - Entitlement называется "premium" (SubscriptionService.ENTITLEMENT_ID)
#     — сверьте точное название в RevenueCat Dashboard -> Entitlements.
#   - Galaxy Store test-покупки работают только на реальном устройстве
#     Samsung с аккаунтом Samsung — эмулятор не поддерживается.
# ============================================================================

set -euo pipefail

echo "=== FinArs: интеграция RevenueCat (Test Store, Google Play + Galaxy Store) ==="
...
if [ ! -d "$PKG_DIR" ]; then
    echo "ERROR: $PKG_DIR не найден. Запустите скрипт из корня репозитория (\$HOME/FA_ksiegowy)."
    exit 1
fi


echo "--- Backing up files that will be modified ---"
[ -f "$PKG_DIR/StoreSource.kt" ] && cp "$PKG_DIR/StoreSource.kt" "$PKG_DIR/StoreSource.kt.bak_${TS}" || true
[ -f "$PKG_DIR/StoreDetector.kt" ] && cp "$PKG_DIR/StoreDetector.kt" "$PKG_DIR/StoreDetector.kt.bak_${TS}" || true
...
[ -f "$PKG_DIR/FaApp.kt" ] && cp "$PKG_DIR/FaApp.kt" "$PKG_DIR/FaApp.kt.bak_${TS}" || true
[ -f "$PKG_DIR/SettingsProActivity.kt" ] && cp "$PKG_DIR/SettingsProActivity.kt" "$PKG_DIR/SettingsProActivity.kt.bak_${TS}" || true
[ -f "$PKG_DIR/PurchaseVerifier.kt" ] && cp "$PKG_DIR/PurchaseVerifier.kt" "$PKG_DIR/PurchaseVerifier.kt.bak_${TS}" || true
[ -f "$RES_DIR/layout/activity_settings_pro.xml" ] && cp "$RES_DIR/layout/activity_settings_pro.xml" "$RES_DIR/layout/activity_settings_pro.xml.bak_${TS}" || true
[ -f "$RES_DIR/values/strings.xml" ] && cp "$RES_DIR/values/strings.xml" "$RES_DIR/values/strings.xml.bak_${TS}" || true
[ -f "$RES_DIR/values-ru/strings.xml" ] && cp "$RES_DIR/values-ru/strings.xml" "$RES_DIR/values-ru/strings.xml.bak_${TS}" || true
[ -f "$RES_DIR/values-pl/strings.xml" ] && cp "$RES_DIR/values-pl/strings.xml" "$RES_DIR/values-pl/strings.xml.bak_${TS}" || true
[ -f "app/build.gradle" ] && cp "app/build.gradle" "app/build.gradle.bak_${TS}" || true

echo "Writing $PKG_DIR/StoreSource.kt"
cat > "$PKG_DIR/StoreSource.kt" << 'FINARS_EOF'
...
echo "Writing $PKG_DIR/StoreDetector.kt"
cat > "$PKG_DIR/StoreDetector.kt" << 'FINARS_EOF'
package com.example.fa_ksiegowy
...
echo "Writing $PKG_DIR/FaApp.kt"
cat > "$PKG_DIR/FaApp.kt" << 'FINARS_EOF'
package com.example.fa_ksiegowy
...
echo "Writing $PKG_DIR/PurchaseVerifier.kt"
cat > "$PKG_DIR/PurchaseVerifier.kt" << 'FINARS_EOF'
package com.example.fa_ksiegowy
...
echo "Writing $RES_DIR/layout/activity_settings_pro.xml"
cat > "$RES_DIR/layout/activity_settings_pro.xml" << 'FINARS_EOF'
...
echo "Writing $RES_DIR/values/strings.xml"
cat > "$RES_DIR/values/strings.xml" << 'FINARS_EOF'
<?xml version="1.0" encoding="utf-8"?>
<resources>
...
echo "Writing $RES_DIR/values-ru/strings.xml"
cat > "$RES_DIR/values-ru/strings.xml" << 'FINARS_EOF'
<?xml version="1.0" encoding="utf-8"?>
<resources>
    <string name="app_name">FinArs</string>
    <string name="app_subtitle">FinArs</string>
...
echo "Writing $RES_DIR/values-pl/strings.xml"
cat > "$RES_DIR/values-pl/strings.xml" << 'FINARS_EOF'
<?xml version="1.0" encoding="utf-8"?>
<resources>
    <string name="app_name">FinArs</string>
    <string name="app_subtitle">FinArs</string>
...
echo ""
echo "--- Проверка баланса скобок в изменённых Kotlin-файлах ---"
CHECK_FAILED=0
for f in StoreSource.kt StoreDetector.kt SubscriptionService.kt BillingManager.kt FaApp.kt SettingsProActivity.kt PurchaseVerifier.kt; do
    if python3 - "$PKG_DIR/$f" << 'PYEOF'
import sys
path = sys.argv[1]
...
while i < len(s):
    ...
    if c in '([{':
        ...
        if not stack or pairs[c] != stack[-1][0]:
            print(f"{path}: MISMATCH at line {line}, char '{c}'")
            sys.exit(1)
        stack.pop()
    i += 1
if stack:
...
if [ "$CHECK_FAILED" -ne 0 ]; then
    echo "ERROR: обнаружены синтаксические проблемы (баланс скобок) — остановка без коммита."
    exit 1
fi
...
echo ""
echo "--- Проверка build.gradle (баланс фигурных скобок) ---"
python3 -c "
s = open('app/build.gradle').read()
o, c = s.count('{'), s.count('}')
print('open =', o, 'close =', c)
if o != c:
    raise SystemExit('build.gradle brace mismatch')
"

echo ""
echo "--- Все проверки пройдены ---"
echo ""
echo "Изменённые/новые файлы:"
git status --short
...
if git diff --cached --quiet; then
    echo "Нет изменений для коммита (файлы уже идентичны текущему состоянию репозитория)."
else
    git commit -m "Integrate RevenueCat Test Store with global Pro features unlock"
...
echo ""
echo "=== Напоминание ==="
echo "Сейчас активен ТОЛЬКО тестовый ключ RevenueCat (Test Store)."
echo "После проверки покупок впишите боевые ключи Google Play / Galaxy Store"
echo "в $PKG_DIR/SubscriptionService.kt (GOOGLE_PLAY_API_KEY / GALAXY_STORE_API_KEY)."
