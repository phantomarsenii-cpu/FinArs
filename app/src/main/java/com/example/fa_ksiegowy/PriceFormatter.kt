 ...
 * not the currency's home locale. Java's [java.util.Currency.getSymbol]
 * falls back to the raw ISO code (e.g. "PLN") whenever the running locale
 * has no localized symbol for that currency — so a Polish (PLN) purchase
 * shown on a device set to Russian can render as "139,99 PLN" instead of
 * "139,99 zł". Picking the currency's own home locale for formatting fixes
 * this regardless of the device's language, while still formatting
 * correctly for every other supported currency.
 */
object PriceFormatter {
     ...
     * "$0"), for copy like "Try 7 days for 0 zł". Returns [fallback] when
     * [currencyCode] is null/unknown — e.g. the paywall hasn't loaded a real
     * plan/currency from RevenueCat yet.
     */
