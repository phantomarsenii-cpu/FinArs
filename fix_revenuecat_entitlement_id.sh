...
# FinArs — fix_revenuecat_entitlement_id.sh
# Исправляет ENTITLEMENT_ID: 'premium' -> 'FinArs Pro'
# (точное название Entitlement из RevenueCat Dashboard -> Entitlements).
# Меняет ТОЛЬКО SubscriptionService.kt.
set -euo pipefail

echo "=== FinArs: исправление идентификатора Entitlement RevenueCat ==="
...
echo ""
echo "--- Проверка, что ENTITLEMENT_ID действительно исправлен ---"
if grep -q 'ENTITLEMENT_ID = "FinArs Pro"' "$PKG_DIR/SubscriptionService.kt"; then
    echo "OK: ENTITLEMENT_ID = FinArs Pro на месте"
else
...
if git diff --cached --quiet; then
    ...
    git commit -m "Fix RevenueCat entitlement identifier: premium -> FinArs Pro"
