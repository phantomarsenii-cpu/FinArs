...
echo "=== Обновление 32: кнопка 'Faktury/Фактуры' теперь только для Pro ==="
echo "На главном экране нажатие на 'Faktury' у пользователей без Pro теперь"
echo "показывает диалог 'Функция Pro' с кнопкой перехода в настройки, вместо"
echo "открытия экрана выставления счёта. Для Pro-пользователей всё работает"
echo "как раньше, без изменений."
echo ""
...
if [ ! -f "app/src/main/java/com/example/fa_ksiegowy/MineActivity.kt" ]; then
    echo "!!! Не найден MineActivity.kt — запусти скрипт из корня проекта"
    exit 1
fi

BACKUP_DIR=".update32_backup_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$BACKUP_DIR"
for f in \
...
echo "2) Проверь:"
echo "   - без Pro: нажатие 'Faktury' показывает диалог 'Функция Pro' с кнопкой"
echo "     'Перейти в настройки', экран выставления счёта не открывается"
echo "   - с активным Pro (или через dev-код): 'Faktury' открывает форму счёта как раньше"
echo "3) Если всё ок:"
echo "   git add -A"
echo "   git commit -m 'Make invoice issuing (Faktury) a Pro-only feature'"
echo "   git push"
echo ""
