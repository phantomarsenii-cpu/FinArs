...
echo "=== Obновление 52: prawidlowa dwuetapowa szkala progu podatkowego (0-30k / 30k-120k / 32%) ==="
echo "Co sie zmienia:"
echo " 1) Na glownym ekranie gejdz 'progu podatkowego' byl mylacy: pokazywal"
echo "    JEDNA szkale '.../120 000 zl' z etykieta 'Pierwszy prog podatkowy'"
echo "    - a 120 000 zl to w rzeczywistosci DRUGI prog (12% -> 32%), nie pierwszy."
echo "    Pierwszy prog to kwota wolna od podatku 30 000 zl/rok."
echo " 2) Teraz szkala jest DWUETAPOWA, tak jak realnie dziala skala podatkowa:"
echo "    - Etap 1 (0-30 000 zl): pokazuje ile jeszcze mozna zarobic zanim"
echo "      zacznie sie placic podatek (0%)."
echo "    - Po przekroczeniu 30 000 zl szkala automatycznie przelacza sie na"
echo "      Etap 2 (30 000-120 000 zl): stawka 12%, widac postep do progu 32%."
echo "    - Po przekroczeniu 120 000 zl szkala pokazuje nadwyzke opodatkowana"
echo "      stawka 32%."
echo "    Samo liczenie podatku (TaxHelper.calc) juz bylo poprawne (progresywne"
echo "      0% / 12% / 32% tylko od nadwyzki nad kazdym progiem) - zmienia sie"
echo "      tylko wyswietlanie gejdza, ktore wczesniej nie odzwierciedlalo tego"
echo "      poprawnie."
echo ""
...
if grep -q "limit_bracket_label_tax_free" "app/src/main/res/values/strings.xml" 2>/dev/null; then
    echo "!!! Wyglada na to, ze update_project-52 zostal juz zastosowany (strings.xml ma juz limit_bracket_label_tax_free)"
    exit 1
fi

BACKUP_DIR=".update52_backup_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$BACKUP_DIR"
for f in \
...
echo ""
echo "-> app/src/main/java/com/example/fa_ksiegowy/LimitsHelper.kt"
mkdir -p "$(dirname 'app/src/main/java/com/example/fa_ksiegowy/LimitsHelper.kt')"
cat > 'app/src/main/java/com/example/fa_ksiegowy/LimitsHelper.kt' << 'FAEOF_APP_SRC_MAIN_JAVA_COM_EXAMPLE_FA_KSIEGOWY_LIMITSHELPER_KT'
...
FAEOF_APP_SRC_MAIN_JAVA_COM_EXAMPLE_FA_KSIEGOWY_LIMITSHELPER_KT

echo "-> app/src/main/java/com/example/fa_ksiegowy/MineActivity.kt"
mkdir -p "$(dirname 'app/src/main/java/com/example/fa_ksiegowy/MineActivity.kt')"
cat > 'app/src/main/java/com/example/fa_ksiegowy/MineActivity.kt' << 'FAEOF_APP_SRC_MAIN_JAVA_COM_EXAMPLE_FA_KSIEGOWY_MINEACTIVITY_KT'
package com.example.fa_ksiegowy
...
FAEOF_APP_SRC_MAIN_JAVA_COM_EXAMPLE_FA_KSIEGOWY_MINEACTIVITY_KT
...
echo "  - app/src/main/res/values-ru/strings.xml"
echo ""
echo "Nastepny krok: zbuduj APK jak zwykle (np. push do GitHub -> GitHub Actions),"
echo "lub lokalnie: ./gradlew assembleDebug"
