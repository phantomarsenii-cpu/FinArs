...
# FinArs — fix_revenuecat_yearly_package_id.sh
# Исправляет идентификатор годового пакета: \$rc_yearly -> \$rc_annual
# (в RevenueCat Dashboard годовой стандартный пакет называется $rc_annual,
# не $rc_yearly — подтверждено ошибкой available=[$rc_monthly, $rc_annual]).
# Меняет ТОЛЬКО SubscriptionService.kt.
set -euo pipefail

echo "=== FinArs: исправление идентификатора годового пакета RevenueCat ==="
...
CHECK_FAILED=0
for f in SubscriptionService.kt; do
...
echo ""
echo "--- Проверка, что идентификатор действительно исправлен ---"
if grep -q 'PACKAGE_YEARLY = "\\$rc_annual"' "$PKG_DIR/SubscriptionService.kt"; then
    echo "OK: PACKAGE_YEARLY = $rc_annual на месте"
else
    echo "ERROR: не нашли ожидаемое значение в файле после записи — остановка без коммита."
    exit 1
fi

echo ""
echo "--- git add / commit / push ---"
git add "$PKG_DIR/SubscriptionService.kt"
if git diff --cached --quiet; then
    echo "Нет изменений для коммита."
else
    git commit -m "Fix RevenueCat yearly package identifier: \$rc_yearly -> \$rc_annual"
