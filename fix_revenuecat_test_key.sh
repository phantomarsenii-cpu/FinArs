...
# FinArs — fix_revenuecat_test_key.sh
# Исправляет регистр тестового ключа RevenueCat: Test_... -> test_...
# (SDK вернул 'Invalid API Key' — ключ должен начинаться со строчной 'test_').
# Меняет ТОЛЬКО SubscriptionService.kt.
set -euo pipefail

echo "=== FinArs: исправление регистра тестового ключа RevenueCat ==="
...
echo ""
echo "--- Проверка, что ключ действительно исправлен ---"
if grep -q 'TEST_API_KEY = "test_BFXzgXddRkopsjEDdnaRTNtVXuY"' "$PKG_DIR/SubscriptionService.kt"; then
    echo "OK: ключ test_BFXzgXddRkopsjEDdnaRTNtVXuY на месте"
else
    echo "ERROR: не нашли ожидаемый ключ в файле после записи — остановка без коммита."
    exit 1
fi
...
if git diff --cached --quiet; then
    ...
    git commit -m "Fix RevenueCat Test Store API key casing (Test_ -> test_)"
