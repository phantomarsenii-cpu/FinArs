...
echo "=== Обновление 46: большой пакет изменений ==="
echo "Что меняется:"
echo " 1) Убран сканер чеков (Skanuj paragon + Skanuj paragon z galerii, ML Kit OCR)."
echo "    Остался только обычный Załącz paragon (без автозаполнения)."
echo " 2) Limit zwolnienia z VAT: 200 000 zl/rok -> 240 000 zl/rok (везде: ustawienia,"
echo "    powiadomienia, opis w Info)."
echo " 3) Minimalne wynagrodzenie / Monthly limit (75%) w Ustawieniach teraz widoczne"
echo "    TYLKO dla niezarejestrowanej dzialalnosci - dla kazdego z 3 wariantow"
echo "    Zarejestrowana JDG (skala/liniowy/ryczalt) ten blok jest ukryty."
echo " 4) Stawka ryczaltu usunieta z Ustawien jako jedna wspolna wartosc. Teraz"
echo "    kategoria (3% towar / 5,5% produkt-produkcja / 8,5% uslugi / 12% uslugi IT /"
echo "    14% uslugi medyczne / 17% wolny zawod) wybierana jest dla KAZDEJ operacji"
echo "    osobno - i dla przychodu (Dodaj przychod), i dla kazdej pozycji faktury."
echo "    Podatek ryczaltowy w Historii/Mine/Raporcie/PIT-28 liczony jest teraz"
echo "    osobno dla kazdej kategorii i sumowany (a nie jedna plaska stawka)."
echo " 5) Faktura: sekcja Usluga/towar obsluguje teraz do 20 pozycji (towar i/lub"
echo "    usluga jednoczesnie), kazda z wlasnym Ilosc/Cena i (dla ryczaltu) kategoria."
echo "    Przycisk + Dodaj pozycje dodaje kolejne wiersze; Dodaj towary z magazynu"
echo "    dopisuje pozycje do tego samego kontenera, a nie osobnym polem."
echo " 6) Wszystkie wlasne dialogi (ilosc przy skanowaniu w inwentaryzacji, wybor"
echo "    kategorii ryczaltu) maja teraz spojny, ciemny styl aplikacji zamiast"
echo "    domyslnego systemowego (bialego) AlertDialog."
echo ""
...
BACKUP_DIR=".update46_backup_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$BACKUP_DIR"
for f in \
    "app/src/main/java/com/example/fa_ksiegowy/RyczaltCategory.kt" \
    "app/src/main/java/com/example/fa_ksiegowy/AppDialog.kt" \
    "app/src/main/res/values/ids.xml" \
    "app/src/main/java/com/example/fa_ksiegowy/Entry.kt" \
    "app/src/main/java/com/example/fa_ksiegowy/InvoiceItem.kt" \
    "app/src/main/java/com/example/fa_ksiegowy/AppDatabase.kt" \
    "app/src/main/java/com/example/fa_ksiegowy/TaxHelper.kt" \
    "app/src/main/java/com/example/fa_ksiegowy/Pit36Calculator.kt" \
    "app/src/main/java/com/example/fa_ksiegowy/LimitsHelper.kt" \
...
mkdir -p "$(dirname "app/src/main/java/com/example/fa_ksiegowy/RyczaltCategory.kt")"
cat > app/src/main/java/com/example/fa_ksiegowy/RyczaltCategory.kt << 'EOF_APP_SRC_MAIN_JAVA_COM_EXAMPLE_FA_KSIEGOWY_RYCZALTCATEGORY_KT'
...
EOF_APP_SRC_MAIN_JAVA_COM_EXAMPLE_FA_KSIEGOWY_RYCZALTCATEGORY_KT
echo "OK: app/src/main/java/com/example/fa_ksiegowy/RyczaltCategory.kt"

mkdir -p "$(dirname "app/src/main/java/com/example/fa_ksiegowy/AppDialog.kt")"
cat > app/src/main/java/com/example/fa_ksiegowy/AppDialog.kt << 'EOF_APP_SRC_MAIN_JAVA_COM_EXAMPLE_FA_KSIEGOWY_APPDIALOG_KT'
package com.example.fa_ksiegowy
...
EOF_APP_SRC_MAIN_JAVA_COM_EXAMPLE_FA_KSIEGOWY_APPDIALOG_KT
echo "OK: app/src/main/java/com/example/fa_ksiegowy/AppDialog.kt"

mkdir -p "$(dirname "app/src/main/res/values/ids.xml")"
cat > app/src/main/res/values/ids.xml << 'EOF_APP_SRC_MAIN_RES_VALUES_IDS_XML'
<?xml version="1.0" encoding="utf-8"?>
<resources>
    <!-- Служебные id для View.setTag(id, value) на динамических строках фактуры
         (см. AddInvoiceActivity) — позволяют хранить на одном View сразу два
         независимых "тега" (категория ryczałtu и id товара со склада). -->
    <item name="tag_ryczalt_category" type="id"/>
    <item name="tag_product_id" type="id"/>
</resources>
EOF_APP_SRC_MAIN_RES_VALUES_IDS_XML
echo "OK: app/src/main/res/values/ids.xml"

mkdir -p "$(dirname "app/src/main/java/com/example/fa_ksiegowy/Entry.kt")"
cat > app/src/main/java/com/example/fa_ksiegowy/Entry.kt << 'EOF_APP_SRC_MAIN_JAVA_COM_EXAMPLE_FA_KSIEGOWY_ENTRY_KT'
package com.example.fa_ksiegowy
...
EOF_APP_SRC_MAIN_JAVA_COM_EXAMPLE_FA_KSIEGOWY_ENTRY_KT
echo "OK: app/src/main/java/com/example/fa_ksiegowy/Entry.kt"

mkdir -p "$(dirname "app/src/main/java/com/example/fa_ksiegowy/InvoiceItem.kt")"
cat > app/src/main/java/com/example/fa_ksiegowy/InvoiceItem.kt << 'EOF_APP_SRC_MAIN_JAVA_COM_EXAMPLE_FA_KSIEGOWY_INVOICEITEM_KT'
package com.example.fa_ksiegowy
...
EOF_APP_SRC_MAIN_JAVA_COM_EXAMPLE_FA_KSIEGOWY_INVOICEITEM_KT
echo "OK: app/src/main/java/com/example/fa_ksiegowy/InvoiceItem.kt"
...
cat > app/src/main/java/com/example/fa_ksiegowy/Pit36Calculator.kt << 'EOF_APP_SRC_MAIN_JAVA_COM_EXAMPLE_FA_KSIEGOWY_PIT36CALCULATOR_KT'
package com.example.fa_ksiegowy
...
EOF_APP_SRC_MAIN_JAVA_COM_EXAMPLE_FA_KSIEGOWY_PIT36CALCULATOR_KT
echo "OK: app/src/main/java/com/example/fa_ksiegowy/Pit36Calculator.kt"

mkdir -p "$(dirname "app/src/main/java/com/example/fa_ksiegowy/LimitsHelper.kt")"
cat > app/src/main/java/com/example/fa_ksiegowy/LimitsHelper.kt << 'EOF_APP_SRC_MAIN_JAVA_COM_EXAMPLE_FA_KSIEGOWY_LIMITSHELPER_KT'
package com.example.fa_ksiegowy
...
EOF_APP_SRC_MAIN_JAVA_COM_EXAMPLE_FA_KSIEGOWY_LIMITSHELPER_KT
echo "OK: app/src/main/java/com/example/fa_ksiegowy/LimitsHelper.kt"
...
<LinearLayout
    ...
    <!-- Категория ryczałtu — видна только для приходов при форме JDG_RYCZALT
         (см. AddEntryActivity.updateTypeToggleUi), открывает небольшое стилизованное
         вертикальное меню выбора (см. AppDialog.showOptionPicker). -->
    <Button
...
mkdir -p "$(dirname "app/src/main/res/layout/item_invoice_line.xml")"
cat > app/src/main/res/layout/item_invoice_line.xml << 'EOF_APP_SRC_MAIN_RES_LAYOUT_ITEM_INVOICE_LINE_XML'
<?xml version="1.0" encoding="utf-8"?>
<LinearLayout xmlns:android="http://schemas.android.com/apk/res/android"
    ...
    <!-- Категория ryczałtu для ЭТОЙ позиции — видна только когда в настройках
         выбран ActivityType.JDG_RYCZALT (см. AddInvoiceActivity.addItemRow), так как
         одна фактура может содержать и товар, и услугу с разными ставками. Открывает
         небольшое стилизованное вертикальное меню (см. AppDialog.showOptionPicker). -->
    <Button
...
EOF_APP_SRC_MAIN_RES_LAYOUT_ITEM_INVOICE_LINE_XML
echo "OK: app/src/main/res/layout/item_invoice_line.xml"
...
<resources>
    ...
    <string name="ryczalt_rate_moved_title">Ryczałt rate is now chosen per transaction</string>
    <string name="ryczalt_rate_moved_hint">You no longer set one flat rate here. When adding income or an invoice item, pick the matching category (goods, production, services, IT, medical, freelance) — the correct rate is applied automatically per item.</string>
...
<resources>
    ...
    <string name="ryczalt_rate_moved_title">Stawka ryczałtu wybierana jest teraz dla każdej operacji osobno</string>
    <string name="ryczalt_rate_moved_hint">Nie ustawiasz już tutaj jednej stałej stawki. Dodając przychód lub pozycję faktury, wybierz odpowiednią kategorię (towar, produkcja, usługi, usługi IT, usługi medyczne, wolny zawód) — właściwa stawka zostanie zastosowana automatycznie dla każdej pozycji.</string>
...
<resources>
    ...
    <string name="ryczalt_rate_moved_title">Ставка ryczałtu теперь выбирается для каждой операции отдельно</string>
    <string name="ryczalt_rate_moved_hint">Здесь больше не нужно задавать единую ставку. При добавлении дохода или позиции фактуры выбирайте подходящую категорию (товар, продукция, услуги, IT-услуги, медицинские услуги, свободная профессия) — верная ставка применится автоматически для каждой позиции.</string>
...
echo ""
echo "--- Удаляю ReceiptOcrHelper.kt (сканер чеков убран полностью) ---"
rm -f "app/src/main/java/com/example/fa_ksiegowy/ReceiptOcrHelper.kt"
echo "OK: ReceiptOcrHelper.kt удалён (если существовал)"

echo ""
echo "--- Точечные правки: расчёт ryczałtu по категориям вместо единой ставки ---"
python3 << 'PYEOF'
import sys
changes = [
    ('app/src/main/java/com/example/fa_ksiegowy/HistoryActivity.kt', 'ActivityType.JDG_RYCZALT -> TaxHelper.calcRyczalt(income, ryczaltRate)', 'ActivityType.JDG_RYCZALT -> TaxHelper.calcRyczaltByCategory(allEntries.filter { it.isIncome }, ryczaltRate)'),
    ('app/src/main/java/com/example/fa_ksiegowy/MineActivity.kt', 'ActivityType.JDG_RYCZALT -> TaxHelper.calcRyczalt(income, ryczaltRate)', 'ActivityType.JDG_RYCZALT -> TaxHelper.calcRyczaltByCategory(yearEntries.filter { it.isIncome }, ryczaltRate)'),
    ('app/src/main/java/com/example/fa_ksiegowy/ReportActivity.kt', 'ActivityType.JDG_RYCZALT -> TaxHelper.calcRyczalt(totalIncome, ryczaltRate).tax', 'ActivityType.JDG_RYCZALT -> TaxHelper.calcRyczaltByCategory(entries.filter { it.isIncome }, ryczaltRate).tax'),
]
for path, old, new in changes:
    with open(path, 'r', encoding='utf-8') as f:
        content = f.read()
    if new in content:
        print(f'SKIP (уже применено): {path}')
        continue
    if old not in content:
        print(f'!!! НЕ НАЙДЕНА строка для замены в {path} — проверь файл вручную')
        sys.exit(1)
...
echo ""
echo "=== Обновление 46 применено успешно ==="
echo "База данных: миграция v7 -> v8 (новая необязательная колонка ryczaltCategory"
echo "в entries и invoice_items) — старые данные не теряются."
