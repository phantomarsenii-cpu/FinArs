...
def str_replace_any(path, candidates, new, label):
    """Jak str_replace, ale probuje po kolei kilka mozliwych wariantow 'old'
    (bo build.gradle moze byc w roznym stanie zaleznie od tego, ktore
    wczesniejsze update_project skrypty zostaly juz uruchomione)."""
    with open(path, "r", encoding="utf-8") as f:
        content = f.read()
    if new in content:
...
# Wariant A: oryginalny blok (versionName = tylko data, np. "1.2026.08.12").
VARIANT_A = """// Update: wersja aplikacji byla zahardkodowana (versionCode 1 / "1.0") i nigdy
...
# Wariant B: jesli wczesniej uruchomiony zostal update_project-59-version-name-time.sh
# (versionName = data+godzina, np. "1.2026.08.12-1235").
VARIANT_B = """// Update: wersja aplikacji byla zahardkodowana (versionCode 1 / "1.0") i nigdy
// sie nie zmieniala, wiec Android nie widzial nowej wersji przy kazdej
// instalacji. Liczymy versionCode automatycznie z aktualnego czasu (minuty od
// wlasnego "zerowego" punktu w czasie, zeby liczba zmiescila sie w Int i byla
// mala) — kazdy build ma wiec zawsze wiekszy, unikalny kod. versionName
...
def CUSTOM_EPOCH_MILLIS = 1735689600000L // 2025-01-01 00:00:00 UTC
def autoVersionCode = (int) ((System.currentTimeMillis() - CUSTOM_EPOCH_MILLIS) / 60000L)
// Update: buildDateStamp mial tylko date (yyyy.MM.dd), wiec kilka buildow w
// jednym dniu mialo IDENTYCZNY widoczny numer wersji (Ustawienia -> Aplikacje
// -> FinArs pokazywaly ten sam napis, mimo ze to inne, nowsze APK). Dodajemy
// godzine:minute, zeby kazdy build mial unikalny, czytelny numer wersji.
def buildDateStamp = new Date().format('yyyy.MM.dd-HHmm')"""
