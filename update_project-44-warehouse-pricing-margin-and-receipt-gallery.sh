...
echo "=== Обновление 44: закупка/продажа/наценка товара, цена продажи в фактуре, упущенная выручка при инвентаризации, скан чека из галереи ==="
echo "Что меняется:"
echo " - Magazyn (towar): теперь два поля цены — Cena zakupu (закупка, как раньше) и"
echo "   Cena sprzedaży (продажа) + поле Marża % — все три синхронизированы: меняешь"
echo "   наценку — пересчитывается цена продажи; вписываешь цену продажи вручную —"
echo "   наценка пересчитывается сама, для справки."
echo " - Wybór towarów do faktury: в фактуру подставляется цена ПРОДАЖИ (а не закупки)."
echo " - Inwentaryzacja: в таблице PDF-отчёта и в истории теперь отдельно разница по"
echo "   себестоимости (как раньше) И упущенная/лишняя выручка по цене продажи."
echo " - Skanuj paragon: добавлена кнопка \"Skanuj paragon z galerii\" — фото чека можно"
echo "   выбрать из галереи, а не только снимать камерой; распознаётся тем же OCR."
echo " - Миграция БД v6 -> v7 (без потери данных): products.priceSell/marginPercent,"
echo "   inventory_records.priceSellAtInventory, inventory_sessions.diffValueSell."
echo ""
...
if [ ! -f "app/src/main/java/com/example/fa_ksiegowy/Product.kt" ]; then
    echo "!!! Не найден Product.kt — проверь, что склад (update_project-42) уже применён"
    exit 1
fi

BACKUP_DIR=".update44_backup_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$BACKUP_DIR"
for f in \
    "app/src/main/java/com/example/fa_ksiegowy/Product.kt" \
    ...
    "app/src/main/java/com/example/fa_ksiegowy/AddEditProductActivity.kt" \
    "app/src/main/java/com/example/fa_ksiegowy/SelectProductsActivity.kt" \
...
mkdir -p "$(dirname "app/src/main/java/com/example/fa_ksiegowy/Product.kt")"
cat > app/src/main/java/com/example/fa_ksiegowy/Product.kt << 'EOF_APP_SRC_MAIN_JAVA_COM_EXAMPLE_FA_KSIEGOWY_PRODUCT_KT'
...
EOF_APP_SRC_MAIN_JAVA_COM_EXAMPLE_FA_KSIEGOWY_PRODUCT_KT
echo "OK: app/src/main/java/com/example/fa_ksiegowy/Product.kt"
...
mkdir -p "$(dirname "app/src/main/java/com/example/fa_ksiegowy/AddEditProductActivity.kt")"
cat > app/src/main/java/com/example/fa_ksiegowy/AddEditProductActivity.kt << 'EOF_APP_SRC_MAIN_JAVA_COM_EXAMPLE_FA_KSIEGOWY_ADDEDITPRODUCTACTIVITY_KT'
package com.example.fa_ksiegowy
...
EOF_APP_SRC_MAIN_JAVA_COM_EXAMPLE_FA_KSIEGOWY_ADDEDITPRODUCTACTIVITY_KT
echo "OK: app/src/main/java/com/example/fa_ksiegowy/AddEditProductActivity.kt"

mkdir -p "$(dirname "app/src/main/java/com/example/fa_ksiegowy/SelectProductsActivity.kt")"
cat > app/src/main/java/com/example/fa_ksiegowy/SelectProductsActivity.kt << 'EOF_APP_SRC_MAIN_JAVA_COM_EXAMPLE_FA_KSIEGOWY_SELECTPRODUCTSACTIVITY_KT'
package com.example.fa_ksiegowy
...
EOF_APP_SRC_MAIN_JAVA_COM_EXAMPLE_FA_KSIEGOWY_SELECTPRODUCTSACTIVITY_KT
...
cat > app/src/main/res/layout/activity_add_edit_product.xml << 'EOF_APP_SRC_MAIN_RES_LAYOUT_ACTIVITY_ADD_EDIT_PRODUCT_XML'
...
EOF_APP_SRC_MAIN_RES_LAYOUT_ACTIVITY_ADD_EDIT_PRODUCT_XML
echo "OK: app/src/main/res/layout/activity_add_edit_product.xml"
...
<LinearLayout
    ...
    <Button
        android:id="@+id/btn_scan_receipt_gallery"
        android:layout_width="match_parent"
        android:layout_height="56dp"
        android:layout_marginBottom="28dp"
        android:background="@drawable/input_field_bg"
        android:text="@string/gallery_scan_receipt_button"
        android:textAllCaps="false"
        android:textColor="@color/accent_cyan"
        android:textSize="15sp"
        android:gravity="start|center_vertical"
...
echo "База данных мигрирует автоматически (v6 -> v7), данные не теряются;"
echo "у существующих товаров цена продажи по умолчанию = цене закупки (наценка 0%)."
echo "Бэкап старых файлов лежит в $BACKUP_DIR — если что-то пойдёт не так, можно откатить вручную."
echo ""
echo 'git add -A && git commit -m "Product purchase/sale price + margin %, invoice uses sale price, inventory missed revenue, scan receipt from gallery" && git push'
