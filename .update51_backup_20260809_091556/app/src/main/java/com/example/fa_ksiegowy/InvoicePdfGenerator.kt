...
object InvoicePdfGenerator {
    ...
    private fun buildLabels(context: Context, isVatPayer: Boolean, paymentMethod: PaymentMethod): Labels = Labels(
        // Update: dokument zawsze nazywa się "FAKTURA" (nigdy "RACHUNEK"), niezależnie
    ...
    fun generate(
        ...
        // Update: dedykowany paint dla plakietek prawnych pod danymi sprzedawcy/nabywcy
        // (8pt, szary #555555) — zgodnie ze zgłoszeniem użytkownika (zrzuty ekranu),
        // zamiast wcześniejszego hintPaint (9pt, inny odcień szarości).
        ...
        // Update: plakietka "Osoba fizyczna prowadząca działalność nierejestrowaną
        // (bez NIP)." — TYLKO gdy sprzedawca faktycznie nie jest podatnikiem VAT
        // (brak NIP), bo to jedyny przypadek, w którym to zdanie jest prawdziwe —
        // po ewentualnej rejestracji VAT (przekroczenie limitu 240 000 zł) sprzedawca
        // ma już NIP i ta plakietka by kłamała. Rysowana zawsze STRICTLY pod ostatnią
        // linią bloku sprzedawcy (kontynuacja tego samego `y`, nie osobny blok).
        if (!isVatPayer) {
        ...
        if (!isPhysicalPerson && !buyerNip.isNullOrBlank()) {
            ...
            // Update: naprawiona pozycja plakietki nabywcy — rysowana zawsze jako
            // KOLEJNA linia po adresie (kontynuacja tego samego `y`, tak jak plakietka
            // sprzedawcy powyżej), z dedykowanym legalNotePaint (8pt, #555555) zamiast
            // hintPaint — wcześniej różny rozmiar/kolor tekstu wizualnie "zlewał się"
            // z adresem powyżej (zgłoszenie użytkownika, zrzuty ekranu).
        ...
        // Update: nowy blok, zgłoszenie użytkownika (zrzuty ekranu) — wcześniej
        // dokument w ogóle nie zawierał podstawy prawnej zwolnienia z VAT, mimo że
        // sprzedawca zwolniony podmiotowo jest do tego zobowiązany (art. 106b ust. 2
        // w zw. z art. 113 ustawy o VAT). Rysowany zawsze w lewej części, POD blokiem
        // terminu/sposobu płatności, TYLKO gdy sprzedawca faktycznie nie jest
        // podatnikiem VAT (dla zarejestrowanego podatnika VAT — po przekroczeniu
        // limitu 240 000 zł — ten blok nie ma zastosowania i jest pomijany).
        //
        // Treść jest zawsze po polsku, niezależnie od języka interfejsu aplikacji —
        // to formalna podstawa prawna z polskiej ustawy, tłumaczenie jej na inny
        // język zniekształcałoby dokument prawny (dlatego stringi poniżej istnieją
        // TYLKO w domyślnym values/strings.xml, bez odpowiedników w values-pl/values-ru
        // — obie lokalizacje i tak dziedziczą tę samą, polską wartość domyślną).
        if (!isVatPayer) {
            newPageIfNeeded(40f)
    ...
    /**
     * Update: FAKTURA KORYGUJĄCA — dokument korygujący wcześniej wystawioną
     * fakturę (błędna kwota / zwrot / dopłata). Prostszy układ niż [generate]
     * (bez tabeli pozycji) — pokazuje kwotę PRZED korektą, PO korekcie i deltę,
     * plus przyczynę korekty podaną przez użytkownika. Nawiązuje wizualnie do
     * zwykłej faktury (te same kolory/paleta), ale to celowo osobna, samodzielna
     * funkcja — inny, dużo prostszy układ treści.
     */
    fun generateCorrection(
        ...
        canvas.drawRect(0f, 0f, PAGE_WIDTH.toFloat(), 5f, Paint().apply { color = COLOR_ACCENT })
        if (logoBitmap != null) {
            val logoSize = 52f
            val left = PAGE_WIDTH - MARGIN - logoSize
            val top = MARGIN - 24f
            canvas.drawBitmap(logoBitmap, null, RectF(left, top, left + logoSize, top + logoSize), Paint().apply { isAntiAlias = true; isFilterBitmap = true })
        }
        ...
        fun wrappedLines(text: String, maxCharsPerLine: Int, paint: Paint, gap: Float, x: Float = MARGIN) {
            ...
            if (current.isNotEmpty()) { canvas.drawText(current.toString(), x, y, paint); y += gap }
        }
        ...
        y = maxOf(leftBottomY, rightBottomY) + 24f
